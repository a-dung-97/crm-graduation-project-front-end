<template>
    <el-dialog
        width="30%"
        center
        :title="this.editing ? 'Chỉnh sửa email tự động': 'Thêm email tự động'"
    ...
    >
        <el-form label-position="top" :model="form" :rules="rules" ref="form">
            ...
            <el-form-item v-if="!editing" label="Danh sách email" prop="mailing_list_id">
                <el-select class="w-100" v-model="form.mailing_list_id">
                    <el-option
...
<script>
import { store, update } from "@/api/automation/email-automation";

export default {
    props: ["form", "editing", "showDialog", "mailingLists"],
    data() {
        return {
            loading: false,
            rules: {
                name: [
                    {
                        required: true,
                        message: "Hãy nhập tên email tự động",
                        trigger: "blur"
                    }
                ],
                mailing_list_id: [
